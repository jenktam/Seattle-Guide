...
function renderNearbyLocations(map, currentLatitude, currentLongitude) {
  __WEBPACK_IMPORTED_MODULE_0_axios___default.a.get(`/api/yelp/${currentLatitude}/${currentLongitude}`).then(locations => {
...
function renderCurrentLocation(map, currentLatitude, currentLongitude) {
  ...
  const currentLocation = L.marker([currentLatitude, currentLongitude], { icon: greenIcon }).bindPopup(`<b>Current Location</b></br>This is your current location.`).addTo(map);
}
